    ...
    <div class="header">
        <div class="header-wrapper">
            <div class="header-left">
                ...
                <div class="greeting">Dear Correlate,</div>
                <div class="greeting">Kindly provide the mentioned services to our esteemed guests in the exchange of this voucher.</div>
            </div>
    ...
    <div class="section-divider">
        ...
        <table class="flight-table">
            ...
            <tbody>
                ...
                <tr>
                    <td><strong>{{ $flight->mode ?? 'N/A' }}</strong></td>
                    <td><strong>{{ $flight->info ?? 'N/A' }}</strong></td>
                    <td><strong>{{ $flight->from_city ?? 'N/A' }}</strong></td>
                    <td><strong>{{ $flight->to_city ?? 'N/A' }}</strong></td>
                    <td><strong>
                        @if($flight->departure_date && $flight->departure_time)
                            {{ \Carbon\Carbon::parse($flight->departure_date)->format('d-M') }} {{ \Carbon\Carbon::parse($flight->departure_time)->format('H:i') }} Hrs
                        @else
                            N/A
                        @endif
                    </strong></td>
                    <td><strong>
                        @if($flight->arrival_date && $flight->arrival_time)
                            {{ \Carbon\Carbon::parse($flight->arrival_date)->format('d-M') }} {{ \Carbon\Carbon::parse($flight->arrival_time)->format('H:i') }} Hrs
                        @else
                            N/A
                        @endif
                    </strong></td>
    ...
    <div class="section-divider">
        ...
        <table class="hotel-contact-table">
            <tr>
                ...
                <td><strong>+971 4 507 9000</strong></td>
            </tr>
            <tr>
                <td><strong>Address</strong></td>
                <td colspan="3"><strong>{{ $accommodation->location ?? 'N/A' }}</strong></td>
            </tr>
        </table>
    </div>
    @endif

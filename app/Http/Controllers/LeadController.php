...
class LeadController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Default to 'booked' status if no status filter is provided
    ...
    public function update(Request $request, Lead $lead)
    {
        ...
        $request->has('post_sales_user_id') ||
        $request->has('operations_user_id') ||
        $request->has('ticketing_user_id') ||
        $request->has('visa_user_id') ||
        $request->has('insurance_user_id') ||
        $request->has('accountant_user_id') ||
        $request->has('delivery_user_id');
        ...
        if ($isBookingForm) {
            ...
            $validated = $request->validate([
                ...
                'post_sales_user_id' => 'nullable|exists:users,id',
                'operations_user_id' => 'nullable|exists:users,id',
                'ticketing_user_id' => 'nullable|exists:users,id',
                'visa_user_id' => 'nullable|exists:users,id',
                'insurance_user_id' => 'nullable|exists:users,id',
                'accountant_user_id' => 'nullable|exists:users,id',
            ...
            // Add department assignments to updateData
            $deptFields = [
                'post_sales_user_id',
                'operations_user_id',
                'ticketing_user_id',
    ...
    public function updateReassignedUser(Request $request, Lead $lead)
    {
        ...
        if (!empty($validated['reassigned_employee_id'])) {
            ...
            if ($employee && $employee->user_id) {
                ...
                // If it's a simple User model with no complex mapping needed
                $userId = $validated['reassigned_employee_id'];
            }
        } elseif (!empty($validated['reassigned_to'])) {
            // Backward compatibility: if user_id is provided directly
            $userId = $validated['reassigned_to'];
        ...
        // Add department assignments to update data
        if ($request->has('post_sales_user_id')) $updateData['post_sales_user_id'] = $validated['post_sales_user_id'];
        if ($request->has('operations_user_id')) $updateData['operations_user_id'] = $validated['operations_user_id'];
        if ($request->has('ticketing_user_id')) $updateData['ticketing_user_id'] = $validated['ticketing_user_id'];
        if ($request->has('visa_user_id')) $updateData['visa_user_id'] = $validated['visa_user_id'];
        if ($request->has('insurance_user_id')) $updateData['insurance_user_id'] = $validated['insurance_user_id'];
        if ($request->has('accountant_user_id')) $updateData['accountant_user_id'] = $validated['accountant_user_id'];
        if ($request->has('delivery_user_id')) $updateData['delivery_user_id'] = $validated['delivery_user_id'];
        ...
        return redirect()->back()->with('success', 'Lead reassigned and departments updated successfully!');
    }
